...
_MESSAGES = {
    ...
    'invalid_endpoint': (
        'The endpoints currently available for this api version are /api/v1/plugins and /api/v1/download'
    ),
...
class ViewsTestCase(FrontendTestCase):
    ...
    def test_api_root(self):
        for path in ['/api', '/api/']:
            ...
            self.assert404(response)
            self.assertEqual(response.json, dict(message=_MESSAGES['missing_api_version']))
    ...
    def test_api_v1(self):
        ...
        self.assertEqual(response.json, dict(message=_MESSAGES['invalid_endpoint']))
    ...
    def test_api_v1_plugins_with_id_not_found(self):
        ...
        self.assert404(response)
        self.assertEqual(response.json, dict(error=_MESSAGES['plugin_not_found']))
    ...
    def test_api_v1_download_with_no_id(self):
        ...
        self.assertEqual(response.json, dict(error=_MESSAGES['missing_id'], message=_MESSAGES['download_usage']))
    ...
    def test_api_v2_releases(self):
        ...
        for testkey in ['stable', 'beta', 'dev']:
            ...
            self.assertIn(updates[testkey]['version'][3], ['dev', 'alpha', 'beta', 'rc', 'final'])
            # Url tests
